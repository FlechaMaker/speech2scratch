...
const OpenAI = window.require("openai");
const blockIconURI =
...
class Scratch3Speech2Scratch {
  ...
  constructor(runtime) {
    ...
    this.transcript = "";
    this._mediaRecorderReady = false;
      ...
      navigator.mediaDevices
        ...
        .then((stream) => {
          ...
          this._mediaRecorder.ondataavailable = async (event) => {
            ...
            if (this._audioChunks.length > 0 && this._openai) {
              const blob = new Blob([event.data], {
                type: "audio/webm; codecs=opus",
              });
              const transcription =
                await this._openai.audio.transcriptions.create({
                  file: await OpenAI.toFile(blob, "audio.webm", {
  ...
  setApiKey(args) {
    ...
    console.log("API key set.", apiKey, this._openai);
    return `APIキーを設定しました: ${apiKey}`;
  }

  ready() {
    return this._mediaRecorderReady && this._openai !== null;
  }
  ...
  startRecognition() {
    if (this._mediaRecorder && this._mediaRecorder.state === "inactive") {
      ...
      this._transcriveInterval = setInterval(() => {
